...
class Scraper:
    ...
    def scrape(self):
        ...
        req = Request(initial_link, headers={'User-Agent': 'Mozilla/5.0'})
        new_links = [link for link in BeautifulSoup(self.open_url(req), 'lxml').find_all('a')
    ...
    def _save_data(self, url):
        ...
        req = Request(transcript_link, headers={'User-Agent': 'Mozilla/5.0'})
        transcript = self.open_url(req).read()
        transcript = re.sub('\x00', '', transcript.decode('utf-8'))

        req = Request(meta_link, headers={'User-Agent': 'Mozilla/5.0'})
        meta_page = self.open_url(req)
