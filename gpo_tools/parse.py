...
class Parser:
    def __init__(self, db, user, password, host='localhost', id_values=None):
        ...
        if not id_values:
            ...
            if confirmation == 'y':
                ...
                #write all ids to file for use later
                id_filewriter = csv.writer(open("gpo_tools/data/all_ids.csv", 'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                for id_value in self.id_values:
                    id_filewriter.writerow([id_value])
                print ("FINISHED WRITING ALL IDS TO FILE")
            elif confirmation.startswith("CHRG"):
                self.id_values = [confirmation]
            elif int(confirmation):
                cur.execute('select id from hearings')
                filter_val = "CHRG-" + str(confirmation)
                self.id_values = [r[0] for r in cur.fetchall() if filter_val in r[0]]
            else:
                self.id_values = []
    ...
    def create_dataset(self, out_dir, out_name='corpus', min_token_length=3, min_doc_length=5, min_dic_count=5,
        ...
        index_keys = ['name_raw', 'name_full', 'member_id', 'party', 'state', 'majority', 'party_seniority',
                      ...
                      'date', 'title', 'subcommittee_met', 'committee_met', 'location']
...
class ParseHearing:
    def __init__(self, entry, committee_data, member_table):
        ...
        if self._name_search(self.entry['transcript']) is not None:
            ...
            if any(meta_chamber + '-' + c.strip() not in self.committee_data for c in self.entry['committees']) is True:
                print('Warning! One of the following committees is missing from the committee data file: ')

                for c in self.entry['committees']:
                    print((meta_chamber, c.strip()))
    ...
    def _grab_title(self):
        import re

        return title

    def _grab_location(self):
        import re
    ...
    def _grab_basic_heading_info(self):
        import re
        ...
        #print (self.entry['transcript'][:10000])
        print (list(self.entry.keys()))

        try:
            found_string = re.search("The (Committee|Subcommittee)s? met(.*?) presiding.", self.entry['transcript'], flags=re.DOTALL).group(0).replace("\n", " ")
            print (found_string)


            subcommittee_met = " Subcommittee " in found_string
            committee_met =  " Committee " in found_string
            location = re.search(" in (.*?),", found_string).group(1)
        ...
        #    then series of capital letters and spaces and newlines
        #    then either 
        try:
            title = " ".join(re.search("(\[From the U.S. Government Printing Office\])[A-Z.,0-9\n ]*", self.entry['transcript'], flags=re.DOTALL).group(0).replace("\n", " ").replace("[From the U.S. Government Printing Office]", "").split())
            if title.strip() == "":
                title = " ".join(re.search("(\[From the U.S. Government Printing Office\])[\n ]*[<DOC>[A-Z.,0-9\n ]*", self.entry['transcript'], flags=re.DOTALL).group(0).replace("\n", " ").replace("[From the U.S. Government Printing Office]", "").replace("<DOC>", "").split())
            if title.strip() == "":
                title = " ".join(re.search("(<title>)[A-Z.,0-9\-\n ]*(<\/title>)", self.entry['transcript'], flags=re.DOTALL).group(0).replace("\n", " ").replace("<title>", "").replace("</title>", "").split())
        except:
            try:
                print ("TRYING TITLE TWO")
                title = " ".join(re.search("(\[From the U.S. Government Printing Office\])[\n ]*[<DOC>[A-Z.,0-9\n ]*", self.entry['transcript'], flags=re.DOTALL).group(0).replace("\n", " ").replace("[From the U.S. Government Printing Office]", "").replace("<DOC>", "").split())
            except:
                try:
                    print ("TRYING TITLE THREE")
                    title = " ".join(re.search("(<title>)[A-Z.,0-9\-\n ]*(<\/title>)", self.entry['transcript'], flags=re.DOTALL).group(0).replace("\n", " ").replace("<title>", "").replace("</title>", "").split())
                except:
                    pass

        basic_heading_info = {"title": title, "subcommittee_met": subcommittee_met, "committee_met": committee_met, "location": location}
        print (basic_heading_info)

        return basic_heading_info
        ...
        for i, cut in enumerate(self.statement_cutpoints):
            if len(cut) == 2:
                ...
                if self.committee_data:
                    raw_comittees = [meta_chamber + '-' + c.strip() for c in self.entry['committees']]
                    try:
                        committees = [self.committee_data[meta_chamber + '-' + c.strip()]['Code'] for c in self.entry['committees']]
                        hearing_chamber = list(set([self.committee_data[meta_chamber + '-' + c.strip()]['Chamber'] for c in
                ...
                output.append({'name_raw': name, 'name_full': None, 'member_id': None, 'state': state, 'party': None,
                               ...
                               'leadership': None, 'congress': congress, 'jacket': self.entry['id'], 'raw_comittees': raw_comittees,
                               'cleaned': cleaned})
    ...
    def _assign_metadata(self):
        ...
        for i in range(len(self.parsed)):
            ...
            basic_heading_info = self._grab_basic_heading_info()
            ...
            self.parsed[i].update(
                {'name_full': name_full, 'member_id': member_id, 'party': tuple(party), 'majority': majority,
                 ...
                 'title': basic_heading_info['title'], 'subcommittee_met': basic_heading_info['subcommittee_met'], 'committee_met': basic_heading_info['committee_met'], 'location': basic_heading_info['location']})
